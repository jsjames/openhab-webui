...
const handler = context => {
  return {
    get(obj, prop) {
      ...
      if (['state', 'getters', '_vm', 'toJSON', '__v_isRef'].indexOf(prop.toString()) >= 0)
        return {}
...
const actions = {
  ...
  startTrackingStates(context) {
    ...
    if (context.state.trackerEventSource) {
      ...
      openhab.sse.close(context.state.trackerEventSource)
    ...
    const eventSource = openhab.sse.connectStateTracker(
      ...
      connectionId => {
        ...
        console.debug(
          `Setting initial tracking list (${context.state.trackingList.length} tracked Items): ` +
            trackingListJson
        )
  ...
  updateTrackingList(context, payload) {
    ...
    nextTick(() => {
      ...
      const trackingListJson = JSON.stringify(context.state.trackingList)
      console.debug(
        `Updating tracking list (${context.state.trackingList.length} tracked Items): ` +
          trackingListJson
      )
      openhab.api.postPlain(
        '/rest/events/states/' + context.state.trackerConnectionId,
        trackingListJson,
        'text/plain',
        'application/json'
      )
  ...
  sendCommand(context, { itemName, cmd, updateState }) {
    ...
    return openhab.api.postPlain('/rest/items/' + itemName, cmd, 'text/plain', 'text/plain')
  }
}
