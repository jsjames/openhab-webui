...
export default function (f7) {
  ...
  Blockly.Blocks['oh_color_to_hsb'] = {
    init: function () {
      ...
      this.setTooltip(
        'converts a colour\'s hex rgb representation to openHAB\'s hue-saturation-brightness string'
      )
      this.setHelpUrl(
  ...
  javascriptGenerator.forBlock['oh_color_to_hsb'] = function (block) {
    ...
    const hexColor = javascriptGenerator.valueToCode(
      block,
      'hexColor',
  ...
  Blockly.Blocks['oh_color_item'] = {
    init: function () {
      ...
      this.setHelpUrl(
        'https://www.openhab.org/docs/configuration/blockly/rules-blockly-standard-ext.html#item-color'
      )
  ...
  javascriptGenerator.forBlock['oh_color_item'] = function (block) {
    ...
    const code =
      inputType === 'oh_item'
        ? `items.getItem(${theItem}).rawState.get${attributeName}()`
        : `${theItem}.rawState.get${attributeName}()`
    return [code, 0]
  }
