...
const generateCodeForBlock = block => {
  ...
  const codeComponent =
    definition.slots && definition.slots.code && definition.slots.code[0]
      ? definition.slots.code[0]
      : null
  ...
  const provideUtility = utilityName => {
    let utilityCode = [
      `function ${javascriptGenerator.FUNCTION_NAME_PLACEHOLDER_}() { /* error! */ }`
    ]
    if (library.slots.utilities) {
      const utilityComponent = library.slots.utilities.find(
        c => c.config && c.config.name === utilityName
      )
      if (!utilityComponent) {
      } else {
        switch (utilityComponent.component) {
          ...
          case 'UtilityJavaType':
            ...
            break
          default:
            utilityCode = utilityComponent.config.code.replace(
              '{{name}}',
    ...
    if (placeholderFields.length >= 2) {
      ...
      switch (placeholderType) {
        ...
        case 'input':
          ...
          context.inputs[placeholderName] = javascriptGenerator.valueToCode(
            block,
            placeholderName,
            order
          )
        ...
        case 'temp_name':
          if (!context.uniqueIdentifiers[placeholderName]) {
            const realm = placeholderOption
              ? Blockly.Variables[placeholderOption]
              : Blockly.Variables.NAME_TYPE
            context.uniqueIdentifiers[placeholderName] =
              javascriptGenerator.variableDB_.getDistinctName(placeholderName, realm)
          }
          return code.replace(placeholder, context.uniqueIdentifiers[placeholderName])
        case 'statements':
          if (!context.statements[placeholderName]) {
            context.statements[placeholderName] = javascriptGenerator.statementToCode(
              block,
              placeholderName
            )
          }
          return code.replace(
            placeholder,
            context.statements[placeholderName].replace(/^ {2}/, '').trim()
          )
        default:
  ...
  if (block.outputConnection) {
    const order = codeComponent.config.order
      ? javascriptGenerator.forBlock[codeComponent.config.order]
      : javascriptGenerator.ORDER_NONE
    return [code, order]
  } else {
    return code
