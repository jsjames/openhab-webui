...
export default function (f7) {
  ...
  Blockly.Blocks['oh_ephemeris_check'] = {
    init: function () {
      ...
      this.setHelpUrl(
        'https://www.openhab.org/docs/configuration/blockly/rules-blockly-ephemeris.html#holiday-weekend-or-weekday-check'
      )
    }
  }
  ...
  javascriptGenerator.forBlock['oh_ephemeris_check'] = function (block) {
    const dayInfo = javascriptGenerator.valueToCode(
      block,
      'dayInfo',
  ...
  javascriptGenerator.forBlock['oh_ephemeris_getDaysUntilHoliday'] = function (block) {
    const holidayName = javascriptGenerator.valueToCode(
      block,
      'holidayName',
      javascriptGenerator.ORDER_NONE
    )
