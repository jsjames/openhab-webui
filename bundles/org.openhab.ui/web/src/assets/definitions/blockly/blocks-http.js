...
export default function (f7) {
  ...
  Blockly.Blocks['oh_httprequest'] = {
    init: function () {
      const imageTimeoutField = new Blockly.FieldImage(
        timeoutImage,
        15,
        15,
        undefined,
      ...
      const imageHeaderField = new Blockly.FieldImage(
        headerImage,
        15,
        15,
        undefined,
      ...
      const imageQueryField = new Blockly.FieldImage(
        queryImage,
        15,
        15,
        undefined,
      ...
      this.setHelpUrl(
        'https://www.openhab.org/docs/configuration/blockly/rules-blockly-http.html#requests'
      )
    ...
    updateShape: function (hasTimeout, hasHeader, hasQuery) {
      ...
      if (hasTimeout) {
        if (!timeoutInput) {
          ...
          const blockAfter = this.getInput('requestHeader')
            ? 'requestHeader'
            : this.getInput('query')
              ? 'query'
              : this.getInput('payload')
                ? 'payload'
                : undefined
          if (blockAfter) {
            this.moveInputBefore('timeoutInput', blockAfter)
          }
          timeoutInput.setShadowDom(
            Blockly.utils.xml.textToDom(
              '<shadow type="math_number"><field name="NUM">3000</field></shadow>'
            )
          )
        }
      ...
      if (hasHeader) {
        if (!headerInput) {
          ...
          const blockAfter = this.getInput('query')
            ? 'query'
            : this.getInput('payload')
              ? 'payload'
              : undefined
      ...
      if (['HttpPostRequest', 'HttpPutRequest'].includes(this.requestType)) {
        ...
        if (!hasPayload && payloadInput && payloadInput.type === Blockly.inputs.inputTypes.VALUE) {
          ...
          hasPayload &&
          payloadInput &&
          payloadInput.type === Blockly.inputs.inputTypes.DUMMY
        ) {
          this.removePayloadInput()
          payloadInput = null
        }
        if (!payloadInput) {
          ...
          payloadInput.appendField('with Payload').appendField(
            new Blockly.FieldDropdown(
              [
              ...
              this.handleContentTypeSelection.bind(this)
            ),
            'contentType'
          )
          if (this.contentType) {
        ...
        if (hasPayload) {
          ...
          if (this.contentType === 'application/x-www-form-urlencoded') {
            ...
            payloadInput.setShadowDom(
              Blockly.utils.xml.textToDom(
                '<shadow type="text"><field name="TEXT">payload</field></shadow>'
              )
            )
          }
        }
    ...
    addDictShadowBlock(input, param) {
      input.setShadowDom(
        Blockly.utils.xml.textToDom(`<shadow type="dicts_create_with">
          <mutation items="2" />
  ...
  javascriptGenerator.forBlock['oh_httprequest'] = function (block, generator) {
    ...
    let payload = javascriptGenerator.valueToCode(
      block,
      'payload',
    ...
    const headers = javascriptGenerator.valueToCode(
      block,
      'requestHeader',
      javascriptGenerator.ORDER_ATOMIC
    )
    const timeout =
      javascriptGenerator.valueToCode(block, 'timeoutInput', javascriptGenerator.ORDER_ATOMIC) ||
      3000
