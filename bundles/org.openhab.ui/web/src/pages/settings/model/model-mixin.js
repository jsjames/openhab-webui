          ...
          if (this.includeNonSemantic) {
            this.rootGroups = this.items
              .filter(
                i =>
                  i.type === 'Group' &&
                  (!i.metadata || !i.metadata.semantics) &&
      ...
      if (this.includeNonSemantic) {
        parent.children.groups = this.items
          .filter(
            i =>
              i.type === 'Group' &&
              !(parent.item.metadata && parent.item.metadata.semantics) &&
        ...
        if (parent.item.metadata && parent.item.metadata.semantics) {
          parent.children.items = this.items
            .filter(
              i =>
                i.type !== 'Group' &&
                (!i.metadata || (i.metadata && !i.metadata.semantics)) &&
                i.groupNames.indexOf(parent.item.name) >= 0
            )
            .map(this.modelItem)
            .sort(compareModelItems)
