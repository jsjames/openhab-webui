  ...
  <f7-page @page:afterin="onPageAfterIn" @page:afterout="onPageAfterOut">
      ...
      :title="
        (createMode
          ? ruleCopy
            ...
            : 'Create rule'
          : stubMode
            ? 'Regenerate rule from template'
            : rule.name) + dirtyIndicator
      "
          ...
          <f7-link @click="save()" v-if="!theme.md">
            {{
              stubMode
                ...
                : $t(createMode ? 'dialogs.create' : 'dialogs.save')
            }}
            <span v-if="$device.desktop">&nbsp;(Ctrl-S)</span>
          </f7-link>
        </template>
        <f7-link
          v-else
    ...
    <f7-tabs class="sitemap-editor-tabs">
      <f7-tab
        ...
        <f7-block
          v-if="ready && rule.status && !createMode && !stubMode"
        ...
        <f7-block v-if="ready" class="block-narrow">
          ...
          <f7-col v-else-if="currentTemplate && stubMode" class="show-associated-template">
            ...
            <f7-list media-list>
              <f7-list-item
                ...
                :footer="currentTemplate.description"
          ...
          <f7-col
            v-if="!hasTemplate || (createMode && ruleCopy?.templateUID && !rule.templateUID)"
            class="rule-modules">
            ...
            <div v-for="section in ['triggers', 'actions', 'conditions']" :key="section">
              <template v-if="isEditable || rule[section].length > 0">
                ...
                <f7-block-footer
                  class="no-margin-top margin-horizontal"
          ...
          <f7-col v-if="!createMode && !stubMode">
            <f7-list>
              <f7-list-button
                v-if="isEditable || !hasOpaqueModule"
                color="blue"
                @click="duplicateRule">
                Duplicate Rule
              </f7-list-button>
    ...
    regenerateFromTemplate() {
      if (this.isEditable) {
        ...
        this.$oh.api
          ...
          .catch(err => {
            f7.dialog.alert(
              'An error occurred when trying to regenerate rule "' +
                this.rule.uid +
                '" from template: ' +
                err
            );
    ...
    addModule(section) {
      ...
      for (
        ...
        ['triggers', 'actions', 'conditions'].some(s =>
          this.rule[s].some(m => m.id === moduleId.toString())
        );
        moduleId++
      );
