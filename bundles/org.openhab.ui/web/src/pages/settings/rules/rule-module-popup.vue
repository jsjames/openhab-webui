    ...
    <f7-page>
      ...
      <f7-block v-if="ruleModule" class="no-margin no-padding">
        ...
        <f7-col v-if="ruleModule.new">
          <f7-block-title
            class="no-margin padding-horizontal margin-vertical"
            v-if="!advancedTypePicker"
            medium
          >
          ...
          <trigger-module-wizard
            v-else-if="!advancedTypePicker && currentSection === 'triggers'"
        ...
        <f7-col
          v-if="ruleModule.type && (!ruleModule.new || advancedTypePicker)"
          class="margin-top"
        >
          <f7-list>
            <f7-list-item
            ...
            >
                ...
                @change="
                  setModuleType(
                    moduleTypes[currentSection].find(
                      t => t.uid === $refs.ruleModuleTypeSmartSelect.f7SmartSelect.getValue()
                    ),
                    true
                  )
                "
              >
                <optgroup
                  v-for="(mt, scope) in groupedModuleTypes(currentSection)"
                  :key="scope"
                  :label="scope"
                >
                  <option
