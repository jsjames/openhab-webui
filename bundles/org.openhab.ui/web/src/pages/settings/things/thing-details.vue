          ...
          <f7-col class="padding-horizontal">
            <div v-show="!error" class="float-right align-items-flex-start align-items-center">
              <f7-link
                ...
                :tooltip="
                  (thing.statusInfo.statusDetail === 'DISABLED' ? 'Enable' : 'Disable') +
        ...
        <f7-block v-if="ready && !error" class="block-narrow">
          <f7-col>
            ...
            <f7-block-footer v-if="!editable" class="no-margin padding-left">
              <f7-icon f7="lock_fill" size="12" color="gray" />&nbsp;Note:
              {{ notEditableMsg }}
            </f7-block-footer>
            <f7-list accordion-opposite>
              ...
              <f7-list-item
                ...
                :badge-color="
                  thing.firmwareStatus.status === 'UPDATE_EXECUTABLE' ? 'green' : 'gray'
                ">
                <f7-accordion-content>
                  <f7-list>
                    <f7-list-item
...
export default {
  ...
  watch: {
    configDirty: function () {
      this.dirty = this.configDirty || this.thingDirty;
    },
    thingDirty: function () {
      this.dirty = this.configDirty || this.thingDirty;
    },
    thing: {
      handler() {
        if (!this.loading) {
          ...
          this.configDirty = !fastDeepEqual(
            thingClone.configuration,
            savedThingClone.configuration
          );
  ...
  methods: {
    ...
    load() {
      ...
      this.$oh.api.get('/rest/things/' + this.thingId).then(data => {
        ...
        const promises = [
          this.$oh.api.get('/rest/thing-types/' + this.thing.thingTypeUID),
          this.$oh.api.get(
            '/rest/channel-types?prefixes=system,' + this.thing.thingTypeUID.split(':')[0]
          ),
        ...
        Promise.all(promises)
          .then(data2 => {
            ...
            this.$oh.api
              .get('/rest/config-descriptions/thing:' + this.thingId)
              .then(data3 => {
                ...
                let bindingActionsNames = bindingActionsGrouped
                  .flatMap(g => g.actions)
                  .flatMap(a => a.name);
                this.configDescriptions.parameters = this.configDescriptions.parameters.filter(
                  p => !bindingActionsNames.includes(p.name)
                ); // params except actions
              ...
              .catch(err => {
                console.log(
                  'No config descriptions for this thing, using those on the thing type: ' + err
                );
    ...
    getBindingActions(configDescriptionsResponse) {
      ...
      let actionContextGroups = configDescriptionsResponse.parameterGroups.filter(
        pg => pg.context === 'actions'
      );
      if (actionContextGroups.length === 0) {
        // No match by context, fall back to heuristic match by group name and label
        actionContextGroups = configDescriptionsResponse.parameterGroups.filter(
          pg => pg.name === 'actions' && pg.label === 'Actions'
        );
      }
    ...
    doConfigAction(action) {
      ...
      let prompt = action.label
        ? `${action.label}?`
        : `Do you want to perform ${action.name} action?`;
    ...
    toggleDisabled() {
      ...
      this.$oh.api
        .putPlain('/rest/things/' + this.thingId + '/enable', enable.toString())
        .then(data => {
          f7.toast
            .create({
    ...
    unlinkAll(removeItems) {
      ...
      f7.dialog.confirm(message, 'Unlink all', () => {
        this.$oh.api.get('/rest/links').then(data => {
          ...
          const unlinkPromises = links.map(l =>
            this.$oh.api.delete(`/rest/links/${l.itemName}/${encodeURIComponent(l.channelUID)}`)
          );
          Promise.all(unlinkPromises)
            .then(() => {
              if (removeItems) {
                dialog.setText('Removing items...');
                const deletePromises = links.map(l =>
                  this.$oh.api.delete(`/rest/items/${l.itemName}`)
                );
    ...
    fromYaml() {
      ...
      const isExtensible = (channel, thingType) => {
        ...
        return (
          thingType.extensibleChannelTypeIds
            .map(t => bindingId + ':' + t)
            .indexOf(channel.channelTypeUID) >= 0
        );
      };

      try {
        if (updatedThing.UID !== this.thing.UID)
          throw new Error('Changing the thing UID is not supported');
        if (updatedThing.thingTypeUID !== this.thing.thingTypeUID)
          throw new Error('Changing the thing type is not supported');
        ...
        if (
          updatedThing.configuration &&
          JSON.stringify(this.thing.configuration) !== JSON.stringify(updatedThing.configuration)
        ) {
          ...
          for (const updatedChannel of updatedThing.channels) {
            ...
            if (existingChannel) {
              if (isExtensible(existingChannel, this.thingType)) {
                if (existingChannel.channelTypeUID)
                  existingChannel.channelTypeUID = updatedChannel.channelTypeUID;
                if (existingChannel.label) existingChannel.label = updatedChannel.label;
                if (existingChannel.description)
                  existingChannel.description = updatedChannel.description;
              }
              if (
                existingChannel.configuration &&
                JSON.stringify(existingChannel.configuration) !==
                  JSON.stringify(updatedChannel.configuration)
              ) {
                existingChannel.configuration = updatedChannel.configuration;
              }
            } else {
              if (!updatedChannel.id || !updatedChannel.label || !updatedChannel.channelTypeUID)
                continue;
              const channelType = this.channelTypes.find(
                ct => ct.UID === updatedChannel.channelTypeUID
              );
            ...
            if (isExtensible(existingChannel, this.thingType)) {
              ...
              if (foundIdx < 0) {
                if (existingChannel.linkedItems && existingChannel.linkedItems.length > 0) {
                  f7.dialog
                    .alert(
                      `Not removing channel ${existingChannel.id} because there are items linked to it`
                    )
                    .open();
                  continue;
                }
