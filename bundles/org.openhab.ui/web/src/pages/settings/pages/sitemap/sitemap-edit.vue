...
export default {
  ...
  data() {
    if (!f7.data.sitemap) f7.data.sitemap = {};
    return {
      includeItemName: f7.data.sitemap.includeItemName || false,
      ready: false,
      loading: false,
      sitemap: {
        uid: 'page_' + utils.id(),
  ...
  computed: {
    hasChildren() {
      ...
      return (
        Array.isArray(this.selectedWidget.slots?.widgets) &&
        this.selectedWidget.slots.widgets.length
      );
    },
    canShowValue() {
      if (!this.selectedWidget) return false;
  ...
  methods: {
    ...
    toggleItemName() {
      ...
      f7.data.sitemap.includeItemName = this.includeItemName;
      this.load();
    },
    save(stay, force) {
      ...
      const promise = this.createMode
        ? this.$oh.api.postPlain(
            '/rest/ui/components/system:sitemap',
    ...
    validateWidgets(stay) {
      ...
      if (Array.isArray(this.sitemap.slots?.widgets) && this.sitemap.slots.widgets.length) {
          ...
          if (siblingIsFrame[siblingIsFrame.length - 1] !== undefined) {
            if (
              ...
              validationWarnings.push(
                'Widget ' + label + ', only frames or no frames at all allowed in linkable widget'
              );
            }
          }
        ...
        widgetList
          ...
          .forEach(widget => {
            if (!widget.slots?.widgets || !widget.slots.widgets.length) {
              let label = scope.widgetErrorLabel(widget.config);
              validationWarnings.push(
                widget.component + ' widget ' + label + ' should not be empty'
              );
            }
          });
        widgetList
          ...
          .forEach(widget => {
            if (!widget.config?.item) {
              let label = scope.widgetErrorLabel(widget.config);
              validationWarnings.push(
                widget.component + ' widget ' + label + ', no item configured'
              );
            }
          });
        widgetList
          ...
          .forEach(widget => {
            if (!widget.config?.url) {
              let label = scope.widgetErrorLabel(widget.config);
              validationWarnings.push(
                widget.component + ' widget ' + label + ', no url configured'
              );
            }
          });
        widgetList
            ...
            if (!(widget.config?.period && this.REGEX_PERIOD.test(widget.config.period))) {
              ...
              validationWarnings.push(
                widget.component +
                  ' widget ' +
                  label +
                  ', invalid period configured: ' +
                  widget.config?.period
              );
            }
            if (
              widget.config?.yAxisDecimalPattern &&
              !this.REGEX_DECIMAL_PATTERN.test(widget.config.yAxisDecimalPattern)
            ) {
              let label = scope.widgetErrorLabel(widget.config);
              validationWarnings.push(
                widget.component +
        ...
        widgetList
          ...
          .forEach(widget => {
            if (
              widget.config?.inputHint &&
              !['text', 'number', 'date', 'time', 'datetime'].includes(widget.config.inputHint)
            ) {
              let label = scope.widgetErrorLabel(widget.config);
              validationWarnings.push(
                widget.component +
        ...
        widgetList
          .filter(widget =>
            ['Slider', 'Setpoint', 'Colortemperaturepicker'].includes(widget.component)
          )
          .forEach(widget => {
            let label = scope.widgetErrorLabel(widget.config);
            if (widget.config?.step <= 0) {
              validationWarnings.push(
                widget.component +
                  ' widget ' +
                  label +
                  ', step size cannot be 0 or negative: ' +
                  widget.config.step
              );
            }
        ...
        widgetList
          ...
          .forEach(widget => {
            ...
            if (!(widget.config?.buttons?.length || widget.slots?.widgets?.length)) {
              validationWarnings.push(
                widget.component + ' widget ' + label + ', no buttons defined'
              );
            }
            let positions = [];
        ...
        widgetList
          ...
          .forEach(widget => {
            ...
            if (!(parentWidget?.component === 'Buttongrid')) {
              validationWarnings.push(
                widget.component +
                  ' widget ' +
                  label +
                  ', can only be defined inside a Buttongrid component'
              );
            }
            if (!widget.config?.item) {
            ...
            if (
              !widget.config?.row ||
              isNaN(widget.config?.row) ||
              widget.config?.row <= 0 ||
              widget.config?.row > 12
            ) {
              validationWarnings.push(
                widget.component +
                  ' widget ' +
                  label +
                  ', invalid row configured: ' +
                  widget.config.row
              );
            }
            if (
              !widget.config?.column ||
              isNaN(widget.config?.column) ||
              widget.config?.column <= 0
            ) {
              validationWarnings.push(
                widget.component +
                  ' widget ' +
                  label +
                  ', invalid column configured: ' +
                  widget.config.column
              );
            }
            if (!widget.config?.cmd) {
              validationWarnings.push(
                widget.component + ' widget ' + label + ', no click command defined'
              );
            }
          });
              ...
              .forEach(attr => {
                widget.config[attr].forEach(param => {
                  if (
                    ...
                    (['visibility', 'valuecolor', 'labelcolor', 'iconcolor', 'iconrules'].includes(
                      attr
                    ) &&
                    ...
                    if (!this.validateMapping(widget.component, param.command)) {
                      validationWarnings.push(
                        widget.component +
                          ' widget ' +
                          label +
                          ', syntax error in button command: ' +
                          param.command
                      );
                    }
                  }
        ...
        if (validationWarnings.length > 0) {
          f7.dialog
            .create({
              ...
              content:
                '<ul style="max-height: 200px; overflow-y: scroll"><li>' +
                validationWarnings.join('</li><li>') +
                '</li></ul>',
              buttons: [
