                    ...
                    <f7-col width="50">
                      ...
                      >
                        ...
                        <f7-block-footer class="margin-top">
                          <small
                            >Position and resize widgets on a grid with fixed dimensions.</small
                          >
                        </f7-block-footer>
                      </f7-link>
                    </f7-col>
...
export default {
  ...
  created() {
    f7.on('svg-onclick-config-update', this.onSvgOnClickConfigUpdate);
  },
  beforeUnmount() {
    f7.off('svg-onclick-config-update', this.onSvgOnClickConfigUpdate);
  },
  methods: {
    addWidget(component, widgetType, parentContext, slot) {
      ...
      if (widgetType) {
        ...
        const addFromModel = () => {
          ...
          f7.once('items-picked', this.doAddFromModel);
          f7.once('model-picker-closed', () => {
            f7.off('items-picked', this.doAddFromModel);
          });

          nextTick(() => actions.destroy());
        };
        const stdWidgets = isList
          ? StandardListWidgets
          : isCells
            ? StandardCellWidgets
            : StandardWidgets;
    ...
    fromYaml() {
      try {
        ...
        if (
          ...
          ((updatedPage.blocks && updatedPage.blocks.length) ||
            (updatedPage.masonry && updatedPage.masonry.length))
        ) {
