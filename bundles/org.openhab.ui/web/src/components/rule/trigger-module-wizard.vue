    ...
    <f7-list :key="thingEventType">
      <f7-list-item
        ...
        <select
          name="thingStatus"
          required
          @change="evt => $set(currentModule.configuration, 'status', evt.target.value)">
          <option
            v-for="status in [
              { value: '', label: '' },
              ...currentModuleType.configDescriptions.find(p => p.name === 'status').options,
            ]"
            :value="status.value"
            :key="status.value"
      ...
      <f7-list-item
        ...
        <select
          name="thingStatus"
          required
          @change="evt => $set(currentModule.configuration, 'previousStatus', evt.target.value)">
          <option
            v-for="status in [
              { value: '', label: '' },
              ...currentModuleType.configDescriptions.find(p => p.name === 'previousStatus')
                .options,
            ]"
            :value="status.value"
            :key="status.value"
