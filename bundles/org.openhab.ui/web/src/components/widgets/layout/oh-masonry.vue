<template>
  <div>
    ...
    <!-- <magic-grid v-if="config.flavor === 'magic-grid'" class="oh-magic-grid" :gap="16" :maxColWidth="300" :animate="false">
      <template v-slot>
        <generic-widget-component class="magic-grid-item" :context="childContext(slotComponent)" v-for="(slotComponent, idx) in context.component.slots.default" :key="idx" v-bind="$attrs" />
      </template>
    </magic-grid> -->
