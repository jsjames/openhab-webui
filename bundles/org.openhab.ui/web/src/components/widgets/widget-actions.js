...
export const actionsMixin = {
  ...
  methods: {
    showActionFeedback(prefix, actionConfig, feedbackConfig) {
      ...
      if (typeof toastConfig === 'string' && toastConfig.startsWith('{'))
        toastConfig = JSON.parse(toastConfig)
        ...
        f7.toast
          .create(
            Object.assign({}, toastConfig, {
              destroyOnClose: true,
              closeTimeout:
                toastConfig.icon || !toastConfig.closeButton ? 2000 : toastConfig.closeTimeout
            })
          )
          .open()
      }
    },
    requestActionConfirmation(prefix, actionConfig, confirmationConfig) {
      let confirmConfig = confirmationConfig || actionConfig[prefix + 'actionConfirmation']
      if (typeof confirmConfig === 'string' && confirmConfig.startsWith('{'))
        confirmConfig = JSON.parse(confirmConfig)
    ...
    performAction(evt, prefix, config, context) {
      ...
      const actionPropsParameterGroup =
        config[(prefix ? prefix + '_' : '') + 'actionPropsParameterGroup']
      const actionConfig = actionPropsParameterGroup
        ? this.evaluateExpression('$props', context.props)
        : config
      prefix = actionPropsParameterGroup
        ? actionPropsParameterGroup.replace(/action/gi, '')
        : prefix
      ...
      if (context.editmode) {
        this.showActionFeedback(
          prefix,
          actionConfig,
          `Action '${action}' not performed while in edit mode`
        )
        return
      }
      if (!action) return
      ...
      const confirmationPromise = actionConfirmation
        ? this.requestActionConfirmation(prefix, actionConfig)
        : Promise.resolve()
        ...
        .then(() => {
          switch (action) {
            ...
            case 'toggle':
              ...
              let cmd =
                context.store[actionToggleItem].state === actionToggleCommand
                  ? actionToggleCommandAlt
                  : actionToggleCommand
              // special behavior for Color, Dimmer
              if (
                actionToggleCommand === 'OFF' &&
                state.split(',').length === 3 &&
                parseInt(state.split(',')[2]) === 0
              )
                cmd = actionToggleCommandAlt
              if (
                actionToggleCommand === 'ON' &&
                state.split(',').length === 3 &&
                parseInt(state.split(',')[2]) > 0
              )
                cmd = actionToggleCommandAlt
              if (actionToggleCommand === 'OFF' && state.indexOf(',') < 0 && parseInt(state) === 0)
                cmd = actionToggleCommandAlt
            ...
            case 'options':
              ...
              const actionsPromise = new Promise((resolve, reject) => {
                if (actionCommandOptions && typeof actionCommandOptions === 'string') {
                  resolve(
                    actionCommandOptions.split(',').map(o => {
                      const option = o.trim()
                      let cmd
                      let label
                      if (option.includes('=')) {
                  ...
                  this.$oh.api.get('/rest/items/' + actionCommandOptionsItem).then(item => {
                    if (item.commandDescription && item.commandDescription.commandOptions) {
                      resolve(
                        item.commandDescription.commandOptions.map(cd => {
                          return {
            ...
            case 'rule':
              ...
              if (typeof actionRuleContext === 'object')
                actionRuleContext = JSON.stringify(actionRuleContext)
              if (!actionRule) break
              this.$oh.api
                .postPlain(
            ...
            case 'sheet':
              ...
              if (
                actionModal.indexOf('page:') !== 0 &&
                actionModal.indexOf('widget:') !== 0 &&
                actionModal.indexOf('oh-') !== 0
              ) {
              ...
              let modalProps = {
                props: {
                  uid: actionModal,
                  el:
                    evt && evt.target && evt.target._icon
                      ? evt.target._icon
                      : evt
                        ? evt.target
                        : null,
            ...
            case 'photos':
              ...
              if (typeof photoBrowserConfig === 'string' && photoBrowserConfig.startsWith('{'))
                photoBrowserConfig = JSON.parse(photoBrowserConfig)
              if (photos && photos.length > 0) {
                const promises = photos.map(el => {
                  ...
                  if (typeof el === 'object') {
                    if (el.item) {
                      return new Promise((resolve, reject) => {
                        self.$oh.api
                          .getPlain(`/rest/items/${el.item}/state`, 'text/plain')
                          .then(data => {
                            resolve({
                              url: data,
                              caption: el.caption
                ...
                Promise.all(promises).then(resolvedPhotos => {
                  ...
                  if (!photoBrowserParams.theme && self.f7.darkTheme)
                    photoBrowserParams.theme = 'dark'
            ...
            case 'group':
              ...
              f7.views.main.router.navigate(groupPopupRoute, {
              ...
              fetch(actionHttpUrl, {
                ...
                .catch(e =>
                  this.showActionFeedback(
                    prefix,
                    actionConfig,
                    `Failed to perform HTTP request: ${e.message}`
                  )
                )
              break
            case 'variable':
              ...
              const actionVariableScope = this.getVariableScope(
                context.ctxVars,
                context.varScope,
                actionVariable
              )
              const actionVariableLocation = actionVariableScope
                ? context.ctxVars[actionVariableScope]
                : context.vars
              if (actionVariableKey) {
                actionVariableValue = this.setVariableKeyValues(
                  actionVariableLocation[actionVariable],
                  actionVariableKey,
                  actionVariableValue
                )
              }
