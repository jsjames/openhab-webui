    ...
    value() {
      if (this.config.variable) {
        ...
        if (this.config.variableKey) {
          return this.getLastVariableKeyValue(
      ...
      const value =
        this.config.ignoreDisplayState === true
          ? this.context.store[this.config.item].state
          : this.context.store[this.config.item].displayState ||
            this.context.store[this.config.item].state
      console.debug(
        `slide-mixin: value changed for ${this.config.item}, displayState: ${this.context.store[this.config.item].displayState}, state: ${this.context.store[this.config.item].state}`
      )
      // use as a brightness control for HSB values
      if (value.split && value.split(',').length === 3) {
        console.debug(
          `slide-mixin: controlling brightness for ${this.config.item}, HSB value: ${value}`
        )
