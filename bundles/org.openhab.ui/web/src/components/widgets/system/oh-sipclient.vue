<template>
  <div class="main-container">
    ...
    <f7-button
      v-else-if="!session || session.isEnded()"
      :style="computedButtonStyle"
      icon-f7="phone_fill_arrow_up_right"
      icon-color="green"
      :icon-size="config.iconSize"
    ...
    <f7-segmented
      v-else-if="session && session.direction === 'incoming' && session.isInProgress()"
    ...
    <f7-segmented v-else style="width: 100%; height: 100%">
      ...
      <f7-button
        v-if="session && session.isInProgress()"
        :style="computedButtonStyle"
        icon-f7="phone_down_fill"
        icon-color="yellow"
        :icon-size="config.iconSize"
          ...
          props: {
            ...
            widget: new WidgetDefinition(
              ...
              pg(
                'sipCredentials',
                'SIP Credentials',
                'These are not stored on the openHAB server, but instead locally in your browser. ' +
              ...
              [
                pt(
                  'username',
                  'Local SIP Username',
                  'Used instead of the username from widget settings and stored on the openHAB server.'
                ),
                pt(
                  'password',
                  'Local SIP Password',
                  'Used instead of the password from the widget settings and stored on the openHAB server.'
                ),
                pt(
                  'ownSipAddress',
                  'Local SIP Address',
                  'SIP Address (phone number) of this local client. Used by the client to remove itself from the dial ' +
      ...
      if (this.config.phonebook !== undefined && this.phonebook.size <= 1) {
        ...
        const actionsPromise = new Promise((resolve, reject) => {
          if (this.phonebook.size > 0) {
            resolve(
              Array.from(this.phonebook.keys())
                .filter(key =>
                  this.config.ownSipAddress ? this.config.ownSipAddress !== key : true
                )
                .map(key => {
                  return {
