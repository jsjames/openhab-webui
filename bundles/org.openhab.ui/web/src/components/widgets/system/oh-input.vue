<template>
  <f7-row no-gap v-if="!config.item || !config.sendButton" class="oh-input" :style="config.style">
    <f7-input
      ...
      :value="
        (config.type && config.type.indexOf('date') === 0) || config.type === 'time'
          ? valueForDatepicker
          : value
      "
  ...
  computed: {
    value() {
      ...
      if (this.config.variable && this.config.variableKey) {
        const keyValue = this.getLastVariableKeyValue(
    ...
    unit() {
      ...
      if (this.config.useDisplayState) {
        const unit = this.extractUnit(
          storeItem.displayState || this.item.stateDescription?.pattern
        )
    ...
    valueIndexInDisplayState() {
      ...
      return parts.findLastIndex(
        part => part.startsWith('%') && part !== '%unit%' && part !== '%%'
      )
    },
    calendarParams() {
  ...
  methods: {
    updated(value) {
      if (this.config.type === 'texteditor') {
        ...
        const oldDate = dayjs(Array.isArray[this.value] ? this.value[0] : this.value).set(
          'millisecond',
          0
        )
        ...
        Array.isArray(value) &&
        this.valueForDatepicker[0].getTime() === value[0].getTime()
      ) {
        return
      }
