...
export default {
  ...
  computed: {
    value() {
      const applyOffset = num =>
        !isNaN(this.config.offset)
          ? Number(this.toStepFixed(num + Number(this.config.offset)))
          : num
