export function compareItems(i1, i2) {
  ...
  // Compare widgetOrder
  const order1 =
    i1.metadata && i1.metadata.widgetOrder ? parseFloat(i1.metadata.widgetOrder.value) : Infinity;
  const order2 =
    i2.metadata && i2.metadata.widgetOrder ? parseFloat(i2.metadata.widgetOrder.value) : Infinity;
...
export function compareParents(i1, i2) {
  ...
  if (i1.modelPath && i2.modelPath) {
    ...
    for (let d = 0; d < minDepth; d++) {
      ...
      const order1 =
        i1.modelPath[d].metadata && i1.modelPath[d].metadata.widgetOrder
          ? i1.modelPath[d].metadata.widgetOrder.value
          : Infinity;
      const order2 =
        i2.modelPath[d].metadata && i2.modelPath[d].metadata.widgetOrder
          ? i2.modelPath[d].metadata.widgetOrder.value
          : Infinity;
      if (order1 !== order2) {
    ...
    if (modelOrder === 0 && i1.modelPath.length !== i2.modelPath.length)
      modelOrder = i1.modelPath.length < i2.modelPath.length ? -1 : 1;
  }
  return modelOrder;
}
