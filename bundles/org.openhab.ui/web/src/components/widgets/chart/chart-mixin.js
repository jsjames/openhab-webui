  ...
  computed: {
    ...
    tooltip() {
      ...
      return this.context.component.slots.tooltip.map(c =>
        OhChartTooltip.get(c, this.startTime, this.endTime, this, this.$device)
      )
    },
    visualMap() {
      ...
      if (this.context.component.slots.visualMap) {
        return this.context.component.slots.visualMap.map(c =>
          OhChartVisualMap.get(c, this.startTime, this.endTime, this, this.$device)
        )
    ...
    legend() {
      ...
      return this.context.component.slots.legend.map(c =>
        OhChartLegend.get(c, this.startTime, this.endTime, this, this.$device)
      )
    },
    title() {
      ...
      for (let i = 0; i < Math.abs(direction); i++) {
        if (chartType) {
          ...
          if (span) {
            day = fn.apply(day, [
              parseInt(span[1]) || 1,
              span[2].replace(/[DWY]/, x => x.toLowerCase())
            ])
          }
        }
      }
