  ...
  computed: {
    ...
    suggestions() {
      return this.history.length > 0
        ? this.history
        : [this.$t('habot.example1'), this.$t('habot.example2'), this.$t('habot.example3')];
    },
  },
  methods: {
    greet() {
    ...
    sendQuery(fromSpeech) {
      ...
      this.$oh.api
        .postPlain(
          '/rest/habot/chat?useCardRegistry=false',
          this.query,
          'application/json',
          'text/plain'
        )
    ...
    convertHABotCard(habotCard) {
      ...
      if (habotCard.slots.media && habotCard.slots.media[0].component === 'HbChartImage') {
        ...
        return Promise.all(
          items.map(i => this.$oh.api.get(`/rest/items/${i}?metadata=semantics,widget`))
        ).then(items => {
          this.card = {
            ...
            slots: {
              default: items.map(i =>
                itemDefaultListComponent(i, { contextLabelSource: 'itemName' })
              ),
            },
          };
          this.busy = false;
