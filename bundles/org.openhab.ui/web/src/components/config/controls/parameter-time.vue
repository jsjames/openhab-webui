...
export default {
  ...
  mounted() {
    ...
    if (this.hasSeconds) {
      cols.push(
        ...
        // Seconds
        {
          values: (() => {
            let arr = []
            for (let i = 0; i <= 59; i = i + (this.configDescription.stepsize % 60)) {
              arr.push(i < 10 ? `0${i}` : i)
            }
            return arr
          })()
    ...
    nextTick(() => {
      this.picker = f7.picker.create({
        ...
        rotateEffect: true,
        value:
          self.value && self.value.indexOf(':') >= 0
            ? self.value.split(':')
            : self.hasSeconds
              ? ['00', '00', '00']
              : ['00', '00'],
  ...
  computed: {
    hasSeconds() {
      return (
        this.configDescription.stepsize !== undefined && this.configDescription.stepsize % 60 !== 0
      )
    }
  },
  methods: {
