...
export default {
  ...
  methods: {
    ...
    emitValues() {
      this.$emit(
        'input',
        this.values.filter((v, idx) => v && this.values.indexOf(v) === idx)
      )
    },
    ...
    gotFocus(event) {
      ...
      const options = this.values?.length
        ? this.options.filter(o => !this.values.some(v => v.toLowerCase() === o.id.toLowerCase()))
        : this.options
