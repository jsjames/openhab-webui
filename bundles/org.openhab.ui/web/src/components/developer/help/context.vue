          ...
          response.text().then(text => {
            import('marked')
              .then(marked => {
                ...
                body = body.replace(
                  /<!--F7:blue ([A-z]*) -->/gm,
                  '<i class="f7-icons size-22" style="color: #2196f3">$1</i>'
                )
                body = body.replace(
                  /<!--F7:green ([A-z]*) -->/gm,
                  '<i class="f7-icons size-22" style="color: #4cd964">$1</i>'
                )
              ...
              .catch(err => {
                this.parsedDocs =
                  '<p>Failed to parse docs.</p><h3>Debug Information</h3><blockquote>' +
                  err +
                  '</blockquote>'
                this.ready = true
              })
