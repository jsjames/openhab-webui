            ...
            >
              ...
              <template #default="{ channelType, channel }" v-else-if="multipleLinksMode">
                <item-picker
                  ...
                  :title="
                    selectedItem(channel) ? 'Change Item Selection' : 'Pick Existing Linked Item'
                  "
                  textColor="blue"
                  :hideIcon="true"
      ...
      if (
        this.thing.channels.filter(
          c =>
            c.label === suffix ||
            (c.channelTypeUID &&
              this.channelTypesMap[c.channelTypeUID] &&
              this.channelTypesMap[c.channelTypeUID].label === suffix)
        ).length > 1
      ) {
        suffix = channel.id
          .replace('#', '_')
