...
const recalculateColumnCount = (windowWidth: number) => {
  ...
  console.log(`Recalculated columns to ${displayColumns.value} based on window width ${windowWidth}px and prop value ${props.columns}`);
};
...
const getChildItems = (): VNode[] => {
  ...
  console.log(`Found ${childItems.length} child items with class 'masonry-grid-item'`);
  return childItems;
};
